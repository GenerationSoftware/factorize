...
describe("pages", () => {
  ...
  it("separates available providers from installed integration management", () => {
    ...
    expect(settings).toContain('id="installed-table" class="mt-5 hidden grid gap-3 sm:grid-cols-2"');
    expect(settings).toContain('<div id="installed-integrations" class="contents"');
    expect(settings).toContain('<article class="relative rounded-2xl');
    expect(settings).toContain('flex items-start justify-between gap-4');
    expect(settings).toContain('<div class="flex min-w-0 items-center gap-2.5">');
    ...
    expect(settings).toContain('<details class="relative shrink-0 text-left">');
