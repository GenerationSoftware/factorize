...
describe("job trigger availability", () => {
  it("only offers universal triggers with zero installations", () => {
    expect(installedTriggerAvailability(false, [], 0)).toEqual({ manual: true, schedule: true, jobLifecycle: true, linear: false, github: false, cloudflareTail: false });
  });

  it("offers each provider for one matching installation", () => {
    expect(installedTriggerAvailability(true, ["active"], 1)).toMatchObject({ linear: true, github: true, cloudflareTail: true });
  });

  it("handles multiple installations without changing the provider result", () => {
    expect(installedTriggerAvailability(true, ["active", "active"], 2)).toMatchObject({ github: true, cloudflareTail: true });
  });

  it("ignores disconnected and stale GitHub installations", () => {
    expect(installedTriggerAvailability(false, ["removed", "suspended"], 0)).toMatchObject({ linear: false, github: false, cloudflareTail: false });
  });
