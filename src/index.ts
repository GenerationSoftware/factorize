...
app.post("/api/pipes", async (c) => {
  const session = await owner(c); if (!session) return c.json({ error: "Unauthorized" }, 401);
  return tenant(c, session.tenantId).fetch("https://tenant/pipes", { method: "POST", headers: { "Content-Type": "application/json" }, body: await c.req.text() });
});
app.put("/api/pipes/:id", async (c) => {
  const session = await owner(c); if (!session) return c.json({ error: "Unauthorized" }, 401);
  return tenant(c, session.tenantId).fetch(`https://tenant/pipes/${encodeURIComponent(c.req.param("id"))}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: await c.req.text() });
});
app.delete("/api/pipes/:id", async (c) => {
  const session = await owner(c); if (!session) return c.json({ error: "Unauthorized" }, 401);
  return tenant(c, session.tenantId).fetch(`https://tenant/pipes/${encodeURIComponent(c.req.param("id"))}`, { method: "DELETE" });
});
