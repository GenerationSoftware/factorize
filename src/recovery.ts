...
export function findOwnedAgent(saved: Partial<HerdrIdentity>, agents: HerdrIdentity[], worktreePath: string, expectedName: string, expectedKind: string): HerdrIdentity | undefined {
  return agents.find((agent) => ownsPane(saved, agent, worktreePath)) ?? agents.find((agent) => agent.name === expectedName && agent.cwd === worktreePath && (!expectedKind || agent.kind === expectedKind));
}
